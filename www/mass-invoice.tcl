...
foreach project_item_id $project_id {
	...
	if {[person::person_p -party_id $recipient_id]} {
	    set rec_organization_list [contact::util::get_employee_organization -employee_id $recipient_id -package_id $contacts_package_id] 
	    if {[llength $rec_organization_list] > 1} {
		if {[lsearch $rec_organization_list $customer_id] > -1} {
		    set rec_organization_id $customer_id
		} else {
